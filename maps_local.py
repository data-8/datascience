...
class Marker(_MapFeature):
    ...
    def map(cls, latitudes, longitudes, labels=None, colors=None, radius=None, **kwargs):
        ...
        if radius is not None:
        	# if the radius is an array then each feature could have
        	# a unique value
        	if isinstance( radius, (int, float) ) == False :
        		assert len(radius) == len(latitudes)
        		inputs.append(radius)
        	else:
        		# only one radius value that should be applied to all
        		# features via kwargs that overides the default value
	        	attrs = {
	            'radius': radius 
	            }
	        	kwargs.update(attrs)
...
class Circle(Marker):
    ...
    def __init__(self, lat, lon, popup='', color='blue', radius=10, **kwargs):
        ...
        #super().__init__(lat, lon, popup, color, radius=radius, line_color="white", **kwargs)
